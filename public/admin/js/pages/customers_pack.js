...
$(document).ready(function() {
    ...
    function add_cus_pack()
    {
    save_method = 'add';
        $('#form')[0].reset(); // reset form on modals
        $('.alert-danger').css('display','none'); // clear error class
        $('#modal_form').modal('show'); // show bootstrap modal
        $('.modal-title').text('Thêm giao dịch mới'); // Set Title to Bootstrap modal title
    }
    function edit_cus_pack(id)
    {
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="usage"]').val(data.usage);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Sửa thông tin giao dịch'); // Set title to Bootstrap modal title
