...
$(document).ready(function() {
    ...
    function add_cus_pay()
    {
    save_method = 'add';
        $('#form')[0].reset(); // reset form on modals
        $('.alert-danger').css('display','none'); // clear error class
        $('#modal_form').modal('show'); // show bootstrap modal
        $('.modal-title').text('Thêm Thanh toán người dùng'); // Set Title to Bootstrap modal title
    }
    function edit_cus_pay(id)
    {
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('.modal-title').text('Sửa thanh toán người dùng'); // Set title to Bootstrap modal title
